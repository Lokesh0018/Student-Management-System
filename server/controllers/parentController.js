...
exports.getParentById = async (req, res) => {
    try {
        ...
        // Convert string of ids to array of strings/numbers
        parentData.studentIds = parentData.studentIds ? parentData.studentIds.split(',') : [];
...
exports.updateParent = async (req, res) => {
    ...
    try {
        ...
        await connection.execute('DELETE FROM parent_student WHERE parent_id = ?', [req.params.id]);

        // Re-insert new relationships
        if (studentIds && Array.isArray(studentIds)) {
            for (const studentId of studentIds) {
                await connection.execute(
                    'INSERT INTO parent_student (parent_id, student_id, relationship) VALUES (?, ?, ?)',
