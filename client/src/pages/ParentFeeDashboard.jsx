    ...
    <div className="page-container">
      ...
      {childrenData.length === 0 ? (
        ...
        childrenData.map(childData => (
          <div key={childData.student.id} className="card" style={{ marginBottom: '20px', background: 'var(--bg)', border: '1px solid var(--border)', padding: '20px' }}>
            <h2 style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '1.4rem', color: 'var(--text-primary)', marginBottom: '20px', borderBottom: '2px solid var(--border)', paddingBottom: '10px' }}>
              <FaUserGraduate className="text-primary" /> {childData.student.name} <span style={{ fontSize: '1rem', color: 'var(--text-secondary)', fontWeight: 'normal' }}>({childData.student.className})</span>
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
              {childData.fees.length === 0 ? (
                <p style={{ color: 'var(--text-secondary)', fontStyle: 'italic' }}>No fees assigned.</p>
              ) : (
                childData.fees.map(fee => (
                  <div key={fee.student_fee_id} style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
                      <h3 style={{ margin: 0, fontSize: '1.15rem', color: 'var(--text-primary)', fontWeight: '600' }}>{fee.term_name}</h3>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', color: 'var(--text-secondary)', fontSize: '0.95rem', marginBottom: '25px' }}>
                      <FaCalendarAlt style={{ marginRight: '8px', color: 'var(--text-muted, #94a3b8)' }} />
                      Due: <strong style={{ marginLeft: '4px', color: 'var(--text-primary)' }}>{new Date(fee.due_date).toLocaleDateString()}</strong>
                    </div>
                    ...
                    <div style={{ marginTop: 'auto', borderTop: '1px solid var(--border)', paddingTop: '20px' }}>
                      {fee.status !== 'PAID' && fee.status !== 'PAYMENT_SUBMITTED' && fee.status !== 'INACTIVE' && fee.status !== 'OVERDUE' ? (
                        ...
                        <div style={{ textAlign: 'center', padding: '10px 0', background: 'var(--bg)', borderRadius: '8px' }}>
                          {fee.status === 'PAID' ? (
